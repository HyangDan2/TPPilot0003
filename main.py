...
try:
    ...
    raise ImportError("onnxruntime가 필요해요. `pip install onnxruntime` 또는 GPU면 `onnxruntime-gpu`") from e
...
# ---------------- Face-only ONNX Detector ----------------
class FaceONNXDetector:
    """
    얼굴 전용 onnx 디텍터 (출력 6채널 가정: [cx, cy, w, h, obj, face])
    - 출력 형태 지원: (1, N, 6), (N, 6), (1, 6, N), (6, N)
    ...
    def _init_session(self):
        ...
        shp = self.sess.get_inputs()[0].shape  # [N,H,W,C] or [N,C,H,W] or dynamic(None)
        if len(shp) == 4:
            if shp[-1] == 3:  # NHWC
    ...
    def _preprocess(self, img_bgr: np.ndarray):
        ...
        if self.expect_nhwc:
            ...
            inp = np.transpose(inp, (2,0,1))  # NCHW
            inp = np.expand_dims(inp, 0)
        return inp, ratio, (dw, dh), img_bgr.shape[:2]

    # ---------- decode (face-only 6ch) ----------
    def _normalize_layout(self, out: np.ndarray) -> np.ndarray:
        """
        out을 (N, 6)로 정규화. 허용 입력:
          (1,N,6) / (N,6) / (1,6,N) / (6,N) / (6,) *k
        """
        if out.ndim == 3:
            # (1, N, 6) or (1, 6, N)
            if out.shape[0] == 1 and out.shape[2] == 6:
                ...
                raise RuntimeError(f"Unexpected 3D output shape {out.shape} (expect (1,N,6) or (1,6,N))")
        elif out.ndim == 2:
            if out.shape[1] == 6:
                # (N, 6)
                pass
                ...
                raise RuntimeError(f"Unexpected 2D output shape {out.shape} (need 6 channels)")
        elif out.ndim == 1:
            if out.shape[0] % 6 != 0:
                raise RuntimeError(f"Unexpected 1D output length {out.shape[0]} (not multiple of 6)")
            out = out.reshape(-1, 6)
        else:
    ...
    def _decode_face6(self, out_any) -> Tuple[np.ndarray, np.ndarray]:
        """
        반환: (boxes_xyxy_input_scale [N,4], conf [N])
        """
        ...
        # xywh -> xyxy (input scale or normalized)
        x, y, w, h = xywh.T
        x1 = x - w*0.5
        y1 = y - h*0.5
        x2 = x + w*0.5
    ...
    def _postprocess(self, outputs, ratio, dwdh, orig_hw):
        # 단일 출력만 오도록 가정(대부분 face 모델은 Detect 1개 텐서)
        if isinstance(outputs, (list, tuple)):
            # 혹시라도 리스트로 나오면 첫 텐서만 사용
            out = outputs[0]
        else:
            out = outputs
        ...
        # 역-Letterbox 복원
        if boxes.shape[0] > 0:
            dh, dw = dwdh[1], dwdh[0]   # dwdh는 (dw, dh)
            boxes[:, [0,2]] -= dw
            boxes[:, [1,3]] -= dh
    ...
    def draw(self, img_bgr: np.ndarray, dets: List[Tuple[np.ndarray, float]]):
        ...
        for box, score in dets:
            ...
            if self.center_score and min(w, h) > self.tiny_box_side_px:
                ...
                cv2.putText(vis, lbl, (x1, max(0, y1-5)), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,0), 2)
        return vis
...
if __name__ == "__main__":
    ...
    ap.add_argument("--model", type=str, required=True, help="face.onnx (Detect 6ch)")
    ap.add_argument("--source", type=str, default="0", help="image/video path or webcam index")
    ap.add_argument("--size", type=int, default=640)
    ap.add_argument("--conf", type=float, default=0.4)
    ap.add_argument("--iou", type=float, default=0.45)
    ap.add_argument("--providers", type=str, default="", help="e.g. CUDAExecutionProvider,CPUExecutionProvider")
    args = ap.parse_args()

    providers = [p for p in args.providers.split(",") if p] or None
    det = FaceONNXDetector(args.model, providers=providers)
    det.input_width = det.input_height = int(args.size)
    det.conf_threshold = float(args.conf)
    det.iou_threshold = float(args.iou)

    def run_frame(bgr):
        dets, ms = det.detect(bgr)
        vis = det.draw(bgr, dets)
        cv2.putText(vis, f"{len(dets)} faces | {ms:.1f} ms", (10,30),
    ...
    if args.source.isdigit() and len(args.source) < 4:
        ...
        while True:
            ...
            if not ok: break
            vis = run_frame(frame)
            cv2.imshow("Face ONNX Detector", vis)
